...
if(isset($_POST['login'])){
    ...
    $s="SELECT * FROM `supervisor` where email='$email' AND `password`='$new_pass'";
    ...
    if($nom_row>0){
        ...
        echo "<script>
        window.location.replace('/agancy/')
        
        
        </script>"; 
    }else{
        $s="SELECT * FROM `employees` where email='$email' AND `password`='$new_pass'";
        $qs=mysqli_query($con,$s);
        $row=mysqli_fetch_assoc($qs);
        $nom_row=mysqli_num_rows($qs);
        if($nom_row>0){
